...
- What hostnames do you want for your {{site.data.reuse.egw}}s? See the guidance about [gateway deployment planning](../planning#gateway-planning) to determine how many hostnames you require for each gateway and gateway group. If you are installing a single gateway, then your gateway must have enough hostnames to match the total number of brokers across all connected clusters. For example, if you have 3 clusters each with 3 brokers, then you must specify at least 9 addresses. Alternatively, you can specify a single address with a wildcard, for example: `*-gwy1-grp1.example.com:9092`.

**Note:** Wildcard hostnames on operator-managed gateways are supported only in {{site.data.reuse.openshift}}.
...
[Generate](#operator-managed-install) your `EventGateway` custom resource YAML file and properties file from the {{site.data.reuse.eem_name}} UI.
