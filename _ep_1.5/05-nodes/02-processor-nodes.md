...
1. Enter an expression in the **Filter Expression** field to filter the events and click **Insert expression**.
   ...
   **Note:** Expressions are prioritized based on [operator precedence](https://calcite.apache.org/docs/reference.html#operators-and-functions){:target="_blank"}.
   ...
   You can update your existing expression in the filter assistant. Open the **Filter assistant**, make your changes, and click **Update expression** to apply them.
...
1. In the **Event time property to detect patterns** drop-down list, select the property of the event that corresponds to an event time. If there is only one input stream and a single event time property, the event time property is automatically selected by default.
