...
**Note:** To auto-register schemas in the {{site.data.reuse.es_name}} schema registry, you need credentials that have operator role permissions (or higher) and permission to create schemas. You can generate credentials by using the {{site.data.reuse.es_name}} [UI](../../security/managing-access/#creating-a-kafkauser-in-the-ibm-event-streams-ui) or [CLI](../../security/managing-access/#creating-a-kafkauser-in-the-ibm-event-streams-cli).
...
4. Set the Java SSL truststore JVM properties to allow the Confluent Platform schema registry client library to make HTTPS calls to the {{site.data.reuse.es_name}} schema registry. For example:\\
