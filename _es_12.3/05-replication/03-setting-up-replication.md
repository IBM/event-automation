...
**Note:** In {{site.data.reuse.es_name}} 12.3.0 and later, the `EventStreamsGeoReplicator` custom resource is deprecated and no longer required. Geo-replication is configured through the {{site.data.reuse.es_name}} UI or CLI, which creates or updates the `KafkaMirrorMaker2` custom resource on the destination cluster. The MirrorMaker2 workers run on the destination cluster and replicate data from the source cluster.
