...
3. Go to the **Set up your IBM MQ Connector** tile, and click **Connect to IBM MQ**.
4. Ensure the **MQ Source** tab is selected, and click **Generate**.
5. Optional: In the **MQ Source connector configuration** window, you can fill the connector configuration before downloading.
...
* When the MQ source connector has been configured to deliver messages to Kafka with exactly-once semantics, ensure that the downstream consumers are only consuming transactionally committed messages. You can do this by setting the [isolation.level](https://kafka.apache.org/42/configuration/consumer-configs/#consumerconfigs_isolation.level){:target="_blank"} configuration property to `read_committed`.
* The IBM MQ source connector must run on Kafka Connect version 3.3.0 or later and the `exactly.once.source.support` property must be set to `enabled` in the Kafka Connect worker configuration. For more information about the `exactly.once.source.support` setting, and the Access Control List (ACL) requirements for the worker nodes, see the [Apache Kafka documentation](https://kafka.apache.org/42/kafka-connect/user-guide/#exactly-once-support){:target="_blank"}. 
...
* Ensure that the IBM MQ source connector principal has a specific set of ACLs to be able to write transactionally to Kafka. See the [Kafka documentation](https://kafka.apache.org/42/kafka-connect/user-guide/#exactly-once-support){:target="_blank"} for the ACL requirements.
