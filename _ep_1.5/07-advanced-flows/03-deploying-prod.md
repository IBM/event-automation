...
- You updated the Flink SQL connector properties and values that are defined in the file `statements.sql` to match your target environment.
  ...
  **Important:** To enable SSL connection for Kafka brokers in production environments, see [configuring SSL for Kafka brokers in production environments](../../installing/configuring/#ssl-for-statements-sql).
...
**Important:** Ensure that the Flink SQL runner JAR file and the `statements.sql` file have read permissions (644) for non-root users. If the JAR file is only readable by the root user, the Flink instance cannot be started by non-root users.
...
1. Modify the [Dockerfile](https://github.com/apache/flink-kubernetes-operator/blob/main/examples/flink-sql-runner-example/Dockerfile){:target="_blank"} to use the IBM Flink image:
   ...
   a. Run the following command to extract the Flink image name, including its SHA digest. For example, if you are running on Flink version {{site.data.reuse.flink_operator_current_version}}:
   ...
   kubectl get deployment \
   -l app.kubernetes.io/name=flink-kubernetes-operator \
   -o jsonpath='{.items[0].spec.template.spec.containers[?(@.name=="flink-kubernetes-operator")].env[?(@.name=="IBM_FLINK_1_20_IMAGE")].value}'
   ```
   ...
   f. The {{site.data.reuse.ep_name}} dependencies that are used by Flink jobs authored by the {{site.data.reuse.ep_name}} UI are in the `ibm-ep-job-dependencies` folder. Copy the `ibm-ep-job-dependencies.jar` from `/opt/flink/ibm-ep-job-dependencies` to `/opt/flink/lib`.
...
2. Create the {{site.data.reuse.ibm_flink_operator}} `FlinkDeployment` custom resource.
   ...
   **Note:** The Flink instance must be [configured](../../installing/planning/#deploying-the-flink-pvc) with persistent storage.
   ...
   d. Explicitly set the Flink version:
   ...
   1. Obtain the correct version by listing the `IBM_FLINK_VERSION` environment variable on the Flink operator pod:
      ...
      kubectl get deployment \
      -l app.kubernetes.io/name=flink-kubernetes-operator \
      -o jsonpath='{.items[0].spec.template.spec.containers[?(@ .name=="flink-kubernetes-operator")].env[?(@.name=="IBM_FLINK_1_20_VERSION")].value}'
      ```
