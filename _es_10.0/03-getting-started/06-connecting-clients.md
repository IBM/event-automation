...
**Note:** You can only use the UI to retrieve the bootstrap address if you installed your {{site.data.reuse.es_name}} instance with authentication enabled.
...
**Note:** You can only use the {{site.data.reuse.es_name}} CLI to retreive the address if your {{site.data.reuse.es_name}} instance has an external listener [configured](../../installing/configuring) in `spec.strimziOverrides.kafka.listeners.external`.
