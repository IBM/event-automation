...
You can upgrade {{site.data.reuse.ep_name}} and the associated {{site.data.reuse.ibm_flink_operator}} to the [latest 1.3.x version]({{ 'support/matrix/#event-processing' | relative_url }}) directly from any earlier 1.3.x or any 1.2.x version by using the latest 1.3.x operators. The upgrade procedure depends on whether you are upgrading to a major, minor, or patch level version, and what your catalog source is.

If you are upgrading from {{site.data.reuse.ep_name}} version 1.1.x or earlier, you must first [upgrade your installation to 1.2.x]({{ 'ep/ep_1.2' | relative_url }}/installing/upgrading/) and then follow these instructions to upgrade to 1.3.x.

- On OpenShift, you can upgrade {{site.data.reuse.ep_name}} and the {{site.data.reuse.ibm_flink_operator}} to the latest 1.3.x version by using the operator channel v1.3. Review the general upgrade [prerequisites](#prerequisites) before following the instructions for [upgrading on OpenShift](#upgrading-on-the-openshift-container-platform).
...
- If your existing Subscription does not use the v1.3 channel, your upgrade is a change in a minor version. Complete the following steps to upgrade:
...
- If your existing Subscription is already on the v1.3 channel, your upgrade is a change to the patch level (third digit) only. [Make the catalog source for your new version available](#making-new-catalog-source-available) to upgrade to the latest level. If you installed by using the IBM Operator Catalog with the `latest` label, new versions are automatically available.

  The {{site.data.reuse.ep_name}} and {{site.data.reuse.ibm_flink_operator}} operators will be automatically upgrade to the latest 1.3.x when they are available in the catalog, and your {{site.data.reuse.ep_name}} and Flink instances are then also automatically upgraded.
...
**Note:** When upgrading to a new major or minor version (such as upgrading from v1.2 to v1.3), ensure you upgrade both the {{site.data.reuse.ep_name}} and the {{site.data.reuse.ibm_flink_operator}} operator, then complete the [post-upgrade tasks](#post-upgrade-tasks), before using your flows again.
...
3. Change the subscription to move to the required update channel, where `vX.Y` is the required update channel (for example, `v1.3`):
...
- If your existing chart version is 1.3.x, your upgrade is a change in the patch level version only, and you only need to follow the steps in [upgrading by using Helm](#upgrading-by-using-helm) to update your Custom Resource Definitions (CRDs) and operator charts to the latest version. The operators will then upgrade your instances automatically.
...
After upgrading your operators to {{site.data.reuse.ep_name}} 1.3.x from version 1.2.x, you must update [the license ID]({{ '/support/licensing/#ibm-event-automation-license-information' | relative_url }}) value in the `spec.license.license` field of your custom resources, depending on the program that you purchased.
