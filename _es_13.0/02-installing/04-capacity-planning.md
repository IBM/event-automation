...
You can use one of the sample configurations provided by {{site.data.reuse.es_name}} to set up a production deployment. For information about the provided samples, see the [sample deployments](../planning/#sample-deployments) section.
...
If required by your planned workload, you can configure node pools with the desired number of brokers, and specify the amount of CPU and memory available to them. For guidance about changing these values, see [scaling](../../administering/scaling/) {{site.data.reuse.es_name}}. 
...
When preparing for your {{site.data.reuse.es_name}} installation, review your workload requirements and consider the configuration options available for performance tuning your {{site.data.reuse.es_name}} installation.

Kafka offers a number of configuration settings that can be adjusted as necessary for an {{site.data.reuse.es_name}} deployment. These can be used to address any bottlenecks in the system as well as perform fine tuning of Kafka performance.
...
- The `num.replica.fetchers` property sets the number of threads available on each broker to replicate messages from topic leaders. Increasing this setting increases I/O parallelism in the follower broker, and can help reduce bottlenecks and message latency. You can start by setting this value to match the number of brokers deployed in the system.\\
  **Note:** Increasing this value results in pool of brokers using more CPU resources and network bandwidth.
...
With Kafka node pools, you can set different storage configurations for each pool. This helps you allocate storage based on the needs of different topics. For example, you can assign topics with high throughput or longer retention to a pool with more storage, and use smaller configurations for less demanding workloads.
...
For more information about these settings, see [the Kafka documentation](https://kafka.apache.org/42/documentation/#configuration){:target="_blank"}.
...
If [message retention](../../getting-started/creating-topics/) is set to long periods and the message volume is high, the storage requirements for the topics could impact the nodes that host the Kafka pods, and cause the nodes to run out of allocated disk space, which could impact normal operation.
