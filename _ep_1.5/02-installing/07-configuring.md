...
- [Flink documentation](https://nightlies.apache.org/flink/flink-kubernetes-operator-docs-release-1.13/docs/custom-resource/reference/#flinkdeployment){:target="_blank"}
...
- [Flink documentation](https://nightlies.apache.org/flink/flink-kubernetes-operator-docs-release-1.13/docs/custom-resource/reference/#flinksessionjob){:target="_blank"}
- [Job and scheduling](https://nightlies.apache.org/flink/flink-docs-release-1.20/docs/internals/job_scheduling/){:target="_blank"}
...
3. In the `spec.flinkConfiguration` section, set the value of `kubernetes.operator.user.artifacts.http.header` field to a custom HTTP header with information about credentials for obtaining the session job artifacts. See the [Flink documentation](https://nightlies.apache.org/flink/flink-kubernetes-operator-docs-release-1.13/docs/operations/configuration/#resourceuser-configuration){:target="_blank"} for more information about custom HTTP headers.
...
If there is a [dynamic storage provisioner](https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/storage/dynamic-provisioning){:target="_blank"} present on the system, {{site.data.reuse.ep_name}} can use it to dynamically provision the persistence.
...
## Configuring SSL for Kafka brokers in production environments
{: #configuring-ssl-for-kafka-brokers-in-production-environments}

When developing flows in the {{site.data.reuse.ep_name}} UI, it is possible to configure the source and sink nodes to make insecure connections to Kafka clusters if the TLS certificates of Kafka brokers could not be verified by {{site.data.reuse.ep_name}}.

If the exported flow contains an insecure configuration, you must configure SSL certificates to securely connect Flink jobs to Kafka brokers in a production environment. 

Based on the export format you use, complete the steps to enable SSL connection to your Kafka brokers.

### For JSON and configuration YAML export format
{: #ssl-for-config-yaml}

When deploying [customized jobs for production or test environments](../../advanced/deploying-customized), follow these steps to enable SSL connection in your `config.yaml` file:

1. Ensure that you have [added the CA certificate](#add-the-ca-certificate-to-the-truststore) for your Kafka brokers to the truststore, and [created a secret](#create-a-secret-with-the-truststore) that includes the truststore.

2. In your `config.yaml` file, remove the insecure SSL configuration `properties.ssl.engine.factory.class: com.ibm.ep.kafka.ssl.TrustAll` and add the following three properties instead:

   ```yaml
   config:
     source:
       source___TABLE:
         properties.ssl.truststore.location: /certs/truststore.<keystore-extension>
         properties.ssl.truststore.password: <chosen-password>
         properties.ssl.truststore.type: <keystore-type>
         # ... other properties
   ```
   Where:

   - `<keystore-extension>` is the extension for your keystore format. For example, `jks` for Java Keystore and `p12` for Public-Key Cryptography Standards.
   - `<keystore-type>` is the type for your keystore format. For example, `JKS` for Java Keystore and `PKCS12` for Public-Key Cryptography Standards.

3. Apply the same truststore configuration to all Kafka source and sink nodes in your `config.yaml` file.

**Important:** Never use `properties.ssl.engine.factory.class: com.ibm.ep.kafka.ssl.TrustAll` in production environments as it disables certificate validation.

### For SQL export format
{: #ssl-for-statements-sql}

When [testing jobs by using the Flink SQL Runner](../../advanced/deploying-production), follow these steps to enable SSL connection in your `statements.sql` file:

1. Ensure that you have [added the CA certificate](#add-the-ca-certificate-to-the-truststore) for your Kafka brokers to the truststore, and [created a secret](#create-a-secret-with-the-truststore) that includes the truststore.

2. In your `statements.sql` file, locate all Kafka connector definitions (both source and sink tables) and remove the insecure SSL configuration line:
   
   ```sql
   'properties.ssl.engine.factory.class' = 'com.ibm.ep.kafka.ssl.TrustAll'
   ```

3. Replace it with the following three properties:
...
4. Apply the same truststore configuration to all Kafka source and sink table definitions in your `statements.sql` file.
