...
When the {{site.data.reuse.es_name}} chart is deployed, an `Internal service error` occurs stating that there are `no ImagePolicies in the "<name>" namespace`, where `<name>` is the namespace into which you are deploying the chart.
...
**Important:** Always ensure you install {{site.data.reuse.es_name}} in a dedicated namespace. Do not use any of the default namespaces (`default`, `kube-system`, `kube-public`, and so on).
