...
Depending on the connector you want to use:\\
   ...
   - Download the [sink connector JAR](../sink/#downloading-the-mq-sink-connector) and [configuration file](../sink/#configuring-the-connector-to-connect-to-mq)
...
For example if running against {{site.data.reuse.es_name}} download the certificate for your install to your IBM z/OS system. Generate an API key that can produce, consume and create topics and update the `connect-standalone.properties` (or `connect-distributed.properties`) file to include:
