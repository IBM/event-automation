...
7. Check the namespace and instance name and click **Delete** to shut down the associated pods and delete the instance.

To delete an {{site.data.reuse.egw}} instance in an {{site.data.reuse.openshift_short}} cluster:
...
7. Check the namespace and instance name, and then click **Delete** to shut down the associated pods and delete the instance.
...
If persistence for the {{site.data.reuse.eem_manager}} instance was enabled but the `deleteClaim` storage property was set to `false`, then you must manually remove the associated Persistent Volumes (PVs) and Persistent Volume Claims (PVCs) that were created at installation time.
...
**Important:** This change causes data to be removed during an upgrade.
...
You can delete an {{site.data.reuse.eem_manager}} instance by using the `kubectl` command-line tool:
...
To delete an operator-managed {{site.data.reuse.egw}} instance by using the `kubectl` command-line tool:

1. {{site.data.reuse.cncf_cli_login}}
2. Ensure that you are in the namespace where your {{site.data.reuse.egw}} instance is installed:
...
Pods initially display a **STATUS** `Terminating` and then be removed from the output as they are deleted.
...
If persistence was enabled for the {{site.data.reuse.eem_manager}} instance but `deleteClaim` storage property was set to `false`, you must manually remove the associated Persistent Volumes (PVs) and Persistent Volume Claims (PVCs) that were created at installation time.
...
**Note:** Take extreme care to select the correct PV name to ensure that you do not delete storage that is associated with a different application instance.
...
1. {{site.data.reuse.cncf_cli_login}}
2. Ensure that you are in the namespace where your {{site.data.reuse.eem_name}} operator is installed:
...
Take caution when you delete CRDs and check that no instances of the CRDs remain, nor other operator installations that use the CRDs.

1. {{site.data.reuse.cncf_cli_login}}
2. Identify the namespace and Helm release that manages the CRDs by looking at the annotations on the CRDs:
...
4. Ensure that you are using the namespace where your {{site.data.reuse.eem_name}} CRD Helm release is located (see previous step).
...
1. {{site.data.reuse.cncf_cli_login}}
2. Ensure that you are using the namespace where your {{site.data.reuse.eem_name}} operator was located:

   ```shell
   kubectl config set-context --current --namespace=<namespace>
   ```

3. List the secrets and identify any that match the instance you deleted. The secrets that are created for {{site.data.reuse.eem_manager}} and {{site.data.reuse.egw}} begin with the name of your {{site.data.reuse.eem_manager}} and {{site.data.reuse.egw}} instances.
...
3. Delete the ConfigMap associated with the gateway:

   ```shell
   kubectl -n <namespace> delete <configmap <gateway group>-<gateway id>-config
   ```


4. Delete any associated Kubernetes Services and Ingresses that you created for your {{site.data.reuse.egw}} instance.

   ```shell
   kubectl -n <namespace> delete <service/ingress>
   ```

<!-- We should consider providing a delete all based on label, which would be a single command. -->