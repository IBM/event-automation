...
**Note:** When applying custom Kafka configuration settings to your {{site.data.reuse.es_name}}, check the [Kafka documentation](https://kafka.apache.org/documentation) to ensure the new configuration settings are consistent and do not cause conflicts.
...
By default, accessing the {{site.data.reuse.es_name}} UI requires an {{site.data.reuse.icpfs}} Identity and Access Management (IAM) user that has been assigned access to {{site.data.reuse.es_name}} (see [managing access](../../security/managing-access/#accessing-the-event-streams-ui-and-cli) for details).
...
This custom resource can be created using the `oc` command or the {{site.data.reuse.openshift_short}} web console under the **Event Streams** operator page.
...
In addition, to enable the collection and display of producer metrics in {{site.data.reuse.es_name}} 11.0.0, also ensure you set the `spec.strimziOverrides.kafka.config.interceptor.class.name` to `com.ibm.eventstreams.interceptors.metrics.ProducerMetricsInterceptor`, for example:
...
![Event Streams 11.0.1 icon]({{ 'images' | relative_url }}/11.0.1.svg "In Event Streams 11.0.1.") **Important:** In {{site.data.reuse.es_name}} 11.0.1 and later, you do not have to set the `spec.strimziOverrides.kafka.config.interceptor.class.name`.
...
`oc annotate --namespace <namespace> secret <instance-name>-cluster-ca eventstreams.ibm.com/ca-key-generation=0`
...
`oc annotate --namespace <namespace> secret <instance-name>-cluster-ca-cert eventstreams.ibm.com/ca-cert-generation=0`
