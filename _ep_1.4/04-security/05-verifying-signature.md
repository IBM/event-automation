...
- Ensure that the following command-line tools are installed on your computer. On Linux systems, these images can typically be installed by using the package manager.
  ...
  - [The OpenShift Container Platform CLI](https://docs.redhat.com/en/documentation/openshift_container_platform/4.20/html/cli_tools/openshift-cli-oc#cli-getting-started){:target="_blank"}
...
4. Run the following command to download, validate, and extract a specific version of the CASE archive.
   ...
   ```shell
   oc ibm-pak get ibm-eventprocessing --version 1.4.7
   ```
   ...
   - Success
   Download of CASE: ibm-eventprocessing, version: 1.4.7 is complete
   ```

   For {{site.data.reuse.ibm_flink_operator}}:

   ```shell
   oc ibm-pak get ibm-eventautomation-flink --version 1.4.7
   ```
   ...
   Parsing inventory items
   - Success
   Download of CASE: ibm-eventautomation-flink, version: 1.4.7 is complete
   ```
...
5. Verify that the CASE archive and images `.csv` files have been generated. For example, ensure you have the following files generated for the CASE.
   ...
   │   │           ├── ibm-eventprocessing-1.4.7-airgap-metadata.yaml
   │   │           ├── ibm-eventprocessing-1.4.7-charts.csv
   │   │           ├── ibm-eventprocessing-1.4.7-images.csv
   │   │           ├── ibm-eventprocessing-1.4.7.tgz
   │   │           └── resourceIndexes
   ...
   │   │   │       ├── charts
   │   │   │       ├── component-set-config.yaml
   │   │   │       ├── ibm-eventautomation-flink-1.4.7-airgap-metadata.yaml
   │   │   │       ├── ibm-eventautomation-flink-1.4.7-charts.csv
   │   │   │       ├── ibm-eventautomation-flink-1.4.7-images.csv
   │   │   │       ├── ibm-eventautomation-flink-1.4.7.tgz
   │   │   │       └── resourceIndexes
...
2. Use a shell script to parse through the CSV file and print out the list of "manifest list images" with their tags. You can use the listed names when pulling and verifying image signatures. In the `tail` command, `~/.ibm-pak/data/cases` represents the default directory where the ibm-pak plug-in will download the CASE archive. Run the following commands to list images by tag:
   ...
   ```shell
   tail -q -n +2 ~/.ibm-pak/data/cases/ibm-eventprocessing/1.4.7/ibm-eventprocessing-1.4.7-images.csv \
   ...
   ```shell
   tail -q -n +2 ~/.ibm-pak/data/cases/ibm-eventautomation-flink/1.4.7/ibm-eventautomation-flink-1.4.7-images.csv \
     | while IFS="," read registry image_name tag digest mtype os arch variant insecure digest_source image_type groups; do
       if [[ "$mtype" == "LIST" ]]; then
         echo "$registry/$image_name:$tag"
       fi
...
4. Create a directory (for example, `images`) for the image. Then use `skopeo` to pull the image into local storage, where `imageName` represents the image name.
   ...
   ```shell
   mkdir images
   skopeo copy docker://icr.io/cpopen/ibm-eventprocessing-operator-catalog:v1.4.7 dir:./images
   ```
...
5. Verify the signature for each required image, where `imageName` is the name of the image and `signature-N` relates to a format for the name.
   ...
   ```shell
   skopeo standalone-verify ./images/manifest.json icr.io/cpopen/ibm-eventprocessing-operator-catalog:v1.4.7 ${fingerprint} ./images/signature-1
   ```
