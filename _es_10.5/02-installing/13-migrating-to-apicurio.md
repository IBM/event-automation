...
2. Update the `EventStreams` custom resource to use the new Apicurio Registry configuration, ensuring not to delete the `spec.schemaRegistry` field. Deleting this field will disrupt any clients currently using the schema registry and may result in schema data loss.
   ...
   **Note:** If you upgrade your {{site.data.reuse.es_name}} version to 10.5.0 or later, the `spec.schemaRegistry` field is not recognized by the {{site.data.reuse.es_name}} operator and is removed from the `EventStreams` custom resource. Any existing schema registry pods and resources will continue to run but will no longer be managed by the the {{site.data.reuse.es_name}} operator and will need to be manually removed after migrating to the Apicurio Registry.
...
4. Migrate the schemas to the Apicurio Registry. Ensure you have logged into the CLI. If prompted to choose a schema registry on login, pick the {{site.data.reuse.es_name}} schema registry:
   ...
   Enter number 2, picking the {{site.data.reuse.es_name}} schema registry.

5. Export your schemas from the {{site.data.reuse.es_name}} schema registry as follows:
