...
Access to the {{site.data.reuse.es_name}} UI requires a Keycloak user with a [role](https://www.ibm.com/docs/en/cloud-paks/cp-integration/2023.4?topic=management-roles-permissions){:target="_blank"} of `eventstreams-admin` or `admin`. The role can be set for the user, or for the group the user is part of.

The default initial administrator (`admin`) user credentials are stored in a secret within the namespace where you  installed the {{site.data.reuse.fs}} operator. Follow the instructions in the [{{site.data.reuse.cp4i}} documentation](https://www.ibm.com/docs/en/cloud-paks/cp-integration/2023.4?topic=management-getting-initial-administrator-password){:target="_blank"} to retrieve the username and password.
...
If Kafka authorization is enabled, operations by IAM users are automatically mapped to a Kafka principal with authorization for the required Kafka resources.
