...
{{site.data.reuse.es_name}} 10.3.x is supported on the {{site.data.reuse.openshift}}.

- Version 10.3.1 is installed by the {{site.data.reuse.es_name}} operator 2.3.1, and includes Kafka version 2.6.2.
- Version 10.3.0 is installed by the {{site.data.reuse.es_name}} operator 2.3.0, and includes Kafka version 2.6.0.
...
The {{site.data.reuse.es_name}} operand will automatically deploy the required {{site.data.reuse.icpfs}} if not present. Ensure you include in your calculations the resource requirements for the following [{{site.data.reuse.fs}} components](https://www.ibm.com/support/knowledgecenter/en/SSHKN6/installer/3.x.x/hardware_sizing_reqs.html){:target="_blank"}:
...
**Note:** If you are installing {{site.data.reuse.es_name}} in an existing {{site.data.reuse.cp4i}} deployment, the required {{site.data.reuse.fs}} might already be installed.
...
{{site.data.reuse.es_name}} requires a [Security Context Constraint (SCC)](https://docs.openshift.com/container-platform/4.6/authentication/managing-security-context-constraints.html){:target="_blank"} to be bound to the target namespace prior to installation.
