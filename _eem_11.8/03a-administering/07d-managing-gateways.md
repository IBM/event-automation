...
To find your gateways in the {{site.data.reuse.eem_name}} UI navigation pane, click **Administration** > **{{site.data.reuse.egw}}s**. 
...
- The endpoints (hostname and port) that Kafka clients use to access the gateway.
...
- **View configuration details**: Access the YAML or Docker command that can be used to re-create the gateway instance. By default, the deployment type that was used at [installation](../../installing/install-gateway) is shown.
...
- **Remove**: Delete the gateway instance.
   ...
   **Note:** If you want to delete a gateway instance, uninstall the gateway first following the steps in [uninstalling](../../installing/uninstalling), and then delete the gateway from the **{{site.data.reuse.egw}}s** page in the UI. 
...
If you are adding a gateway to an existing gateway group on which existing virtual topics are published, then your new gateway must be defined with at least as many hostnames as the total number of brokers across all Kafka clusters that those virtual topics use.
