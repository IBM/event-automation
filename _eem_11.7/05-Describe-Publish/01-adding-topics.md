...
If your Kafka cluster is outside of the scope of your {{site.data.reuse.eem_name}} instance, you can bypass the connection tests. This is particularly useful if your Kafka cluster is located behind a different firewall or is subject to specific network policies that restrict visibility from your {{site.data.reuse.eem_name}} instance. You can add a cluster without connection tests when you [add a topic](#add-topic) or from the [Manage clusters](../../administering/managing-clusters) page. When you add bypass the connection tests, you can define provisional topics to inform {{site.data.reuse.eem_name}} about their existence.
...
1. Optional. Click **Next**. The Editors pane is displayed.
