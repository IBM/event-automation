...
APIs, and API management, are not new concepts. The ability to have governed interfaces which can be socialized broadly allows developers to discover and use APIs to deliver new applications quickly, while the providers of those APIs can manage usage of the APIs as required. API management offerings such as {{site.data.reuse.apic_long}} and {{site.data.reuse.wm_portal_long}} provide a full set of capabilities which provide the following features to manage your APIs:
...
- A single stream of events can be described and exposed in different ways by using [virtual topics](../../about/key-concepts#virtual-topic), allowing streams to provide a stable API to downstream consumers.
...
{{site.data.reuse.eem_name}} can be deployed standalone, and used to manage only a set of event-driven socialization and management requirements. However, you can take advantage of further governance and socialization options when integrating {{site.data.reuse.eem_name}} with API management solutions:

- [Integrate with {{site.data.reuse.apic_short}}](../../integrating-with-apic/overview) for comprehensive API lifecycle and governance management.
- [Integrate with {{site.data.reuse.wm_portal_long}}](../../dpo-integration/overview) to socialize event streams alongside your organization's APIs.

As a result, existing users of these platforms can take full advantage of any existing governance, socialization, and management practices while leveraging the value provided by {{site.data.reuse.eem_name}} to describe, manage, and enforce end-user interaction with event-driven systems.
