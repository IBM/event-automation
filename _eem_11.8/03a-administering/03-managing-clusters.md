...
<!-- _COMMENT: Could we say something about noting down the number of brokers on the new cluster? This is essential information when adding gateways, so the user knows how many gateway addresses to provide._ -->
...
Clusters can be deleted from {{site.data.reuse.eem_name}} if they are no longer in use. Before you can delete a cluster, you must [delete](../../describe/managing-source-topics/#deleting-source-topic) all topics that were added from the cluster.
