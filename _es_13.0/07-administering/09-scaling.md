...
Scaling is the process of adjusting the capacity of your {{site.data.reuse.es_name}} cluster to handle changing workloads. You can scale your cluster in two ways: 

- Manual scaling: You can scale your {{site.data.reuse.es_name}} components by modifying their configuration. For more information, see [manual scaling](#manual-scaling-for-event-streams-components).
- Automatic scaling: You can enable [auto-scaling](#auto-scaling-kafka-brokers), which is supported only for Kafka brokers.

If you want partitions to rebalance automatically whenever brokers are added or removed, enable [auto-rebalancing](#configuring-auto-rebalancing-for-kafka-brokers) before scaling your brokers. After scaling, you can [verify partition reassignment](#verifying-partition-reassignment) to ensure that new brokers received partition assignments and that workloads are balanced.

Before you scale your {{site.data.reuse.es_name}} cluster, review your current configuration. The [prerequisite](../../installing/prerequisites/) guidance gives various examples of different production configurations on which you can base your deployment. To verify it meets your requirements, you should test the system with a workload that is representative of the expected throughput. For this purpose, {{site.data.reuse.es_name}} provides a [workload generator application](../../getting-started/testing-loads/) to test different message loads.
...
Scaling operations require updates to the `EventStreams` custom resource, where broker counts, resource allocations, and related configuration are defined under the `spec.strimziOverrides` property. For more information about modifying these settings, see [modifying installation](../modifying-installation).

## Manual scaling for {{site.data.reuse.es_name}} components
...
For more information about scaling up Kafka node pools, see the [Strimzi documentation](https://strimzi.io/docs/operators/1.0.0/deploying#proc-scaling-up-node-pools-str){:target="_blank"}.

### Increase the CPU request or limit settings for the Kafka brokers
{: #increase-the-cpu-request-or-limit-settings-for-the-kafka-brokers}

The CPU settings for the Kafka brokers are defined in the `EventStreams` custom resource in the `requests` and `limits` properties under `spec.strimziOverrides.nodePools[].resources` (for brokers within a node pool).

You can set different resource configurations for individual Kafka node pools by defining the resource properties under `spec.strimziOverrides.nodePools.resources`. For example:
...
The syntax for these values can be found in the [Kubernetes documentation](https://v1-35.docs.kubernetes.io/docs/concepts/configuration/manage-compute-resources-container/#meaning-of-memory){:target="_blank"}.
...
**Note:** Only a [selected subset](https://strimzi.io/docs/operators/1.0.0/configuring.html#con-common-configuration-jvm-reference){:target="_blank"} of the available JVM options can be configured.
...
You can configure {{site.data.reuse.es_name}} to automatically scale Kafka brokers by creating a [Kubernetes HorizontalPodAutoscaler (HPA)](https://v1-35.docs.kubernetes.io/docs/tasks/run-application/horizontal-pod-autoscale/){:target="_blank"}. After you create an HPA custom resource for Kafka brokers, the HPA scales the number of broker pods based on the metrics that you specify in the custom resource (for example, CPU or memory utilization).
...
- [Cruise Control](../../administering/cruise-control/) is enabled.
- Optional: Consider [enabling tiered storage](../../installing/configuring/#enabling-tiered-storage), which reduces the amount of data moved during rebalancing, and can improve performance when brokers are added or removed.
- [Auto-rebalancing](#configuring-auto-rebalancing-for-kafka-brokers) is enabled. Ensure that all required topics and partitions exist in the cluster so that partitions are distributed correctly. You can verify this by running the `kubectl get kafkatopic -n <namespace>` command.
...
1. Add the following annotation to the broker node pool configuration in your `EventStreams` custom resource to enable HPA support:
   ...
   strimziOverrides:
	...
	```
	 ...
	 Apply your updated `EventStreams` custom resource. The {{site.data.reuse.es_name}} operator no longer manages the replica count for this broker node pool and the HPA takes control of scaling.
...
- Optional: Check the current broker pods.
  Run the following command to see how many broker pods are currently running after auto-scaling:
  ...
  ```
- [Verify partition reassignment](#verifying-partition-reassignment) to ensure that new brokers received partitions after scaling. 
...
After you [initialize the {{site.data.reuse.es_name}} CLI](../../getting-started/logging-in/#logging-in-to-event-streams-cli), run the following command to verify that new brokers received partition assignments after scaling:
