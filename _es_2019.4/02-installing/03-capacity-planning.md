...
When preparing for your {{site.data.reuse.es_name}} installation, consider the performance and capacity requirements for your system.
...
The [prerequisites](../prerequisites/#helm-resource-requirements) for {{site.data.reuse.es_name}} provide information about the minimum resources requirements for a test environment. For a baseline production deployment of {{site.data.reuse.es_name}}, increase the following values.
...
Depending on your workload, you can further scale {{site.data.reuse.es_name}} and fine tune Kafka performance to accommodate the increased requirements.

### Scaling {{site.data.reuse.es_name}}

If required by your planned workload, you can further increase the number of Kafka brokers, and the amount of CPU and memory available to them. For changing other values, see the guidance about [scaling](../../administering/scaling/) {{site.data.reuse.es_name}}.
...
### Tuning {{site.data.reuse.es_name}} Kafka performance

You can further fine-tune the performance settings of your {{site.data.reuse.es_name}} Kafka brokers to suit your requirements. Kafka provides a range of parameters to set, but consider the following ones when reviewing performance requirements. You can set these parameters when installing {{site.data.reuse.es_name}}, or you can modify them later.
...
To set these parameter values, you can use a ConfigMap that specifies Kafka configuration settings for your {{site.data.reuse.es_name}} installation.

#### Setting before installation

If you are creating the ConfigMap and setting the parameters when installing {{site.data.reuse.es_name}}, you can add these parameters to the properties file with the required values.
...
3. When installing {{site.data.reuse.es_name}}, ensure you [provide](../configuring/#specifying-a-configmap-for-kafka-configuration) the ConfigMap to the installation.

#### Modifying an existing installation

If you are updating an existing {{site.data.reuse.es_name}} installation, you can use the ConfigMap you already have for the Kafka configuration settings, and include the parameters and their values in the ConfigMap. You can then apply the new settings by updating the ConfigMap as described [modifying Kafka broker configurations](../../administering/modifying-configs/#modifying-broker-and-cluster-settings), for example:

`helm upgrade --reuse-values --set kafka.configMapName=<configmap_name> <release_name> <charts.tgz>`

Alternatively, you can modify broker configuration settings dynamically by using the {{site.data.reuse.es_name}} CLI as described in [modifying Kafka broker configurations](../../administering/modifying-configs/#modifying-broker-and-cluster-settings), for example:

`cloudctl es cluster-config --config num.replica.fetchers=4`

**Important:** Using the {{site.data.reuse.es_name}} CLI overrides the values specified in the ConfigMap. In addition, the CLI enforces constraints to avoid certain parameters to be misconfigured. For example, you cannot set `num.replica.fetches` to a value greater than double its current value. This means that you might have to make incremental updates to the value, for example:
...
**Important:** You must consider and set these {{site.data.reuse.icp}} configuration options before installing {{site.data.reuse.es_name}}.
...
Without a load balancer, a typical {{site.data.reuse.es_name}} installation includes a master node for allowing external traffic into the cluster. There are also worker nodes that host Kafka brokers. Each broker has an advertised listener that consists of the master node's IP address and a unique node port within the cluster. This means the worker nodes can be identified without being exposed externally.

When a producer connects to the {{site.data.reuse.es_name}} master node through the bootstrap port, they are sent metadata that identifies partition leaders for the topics hosted by the brokers. So, access to the cluster is based on the address `<master_node:bootstrap_port>`, and identification is based on the advertised listener addresses within the cluster, which has a node port to uniquely identify the specific broker.
...
The producer then sends messages to the advertised listener for a partition leader of a topic. These requests go through the master node and are passed to the right worker node in {{site.data.reuse.es_name}} based on the internal IP address for that specific advertised listener that identifies the broker.
...
**Important:** When using a load balancer for {{site.data.reuse.icp}}, ensure you set the address for your endpoint in the [External hostname/IP address field](../configuring/#configuring-external-access) field when installing your {{site.data.reuse.es_name}} instance.
...
You can specify the cluster and topic-level configurations by using [the {{site.data.reuse.es_name}} CLI](../../administering/modifying-configs/#modifying-broker-and-cluster-settings). You can also set topic-level configuration when setting up the topic in the {{site.data.reuse.es_name}} UI (click **Topics** in the primary navigation, then click **Create topic**, and set **Show all available options** to **On**).
