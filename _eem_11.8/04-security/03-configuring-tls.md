...
The {{site.data.reuse.egw}}s secure their endpoint with the `<gateway group>-<gateway name>-ibm-egw-cert` certificate. Kafka clients see this certificate when they connect to the {{site.data.reuse.egw}}. You configure this certificate when you install the {{site.data.reuse.egw}}.

The certificates used to secure mTLS [security controls](../../describe/security-controls) are configured in the {{site.data.reuse.eem_name}} UI. For more information, see [Managing mTLS control certificates](../cred-sets#config-mtls).
