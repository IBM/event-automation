...
3. Configure the ConfigProvider and add the secret as a volume in the `KafkaConnect` custom resource and mount it to a directory in the container.

   **Important:** To use the `${file:...}` syntax in your connector configuration, you must configure the `DirectoryConfigProvider` in the `spec.config` section. Without this configuration, the variable references will not be resolved and your connector will fail.
   ...
   spec:
     ...
     config:
       # REQUIRED: Enable the DirectoryConfigProvider to read secrets from a mounted directory
       # The provider name `file` is used in variable references like ${file:/mnt/mq-credentials:username}
