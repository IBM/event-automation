...
You can specify all the broker configuration options supported by Kafka except those managed directly by {{site.data.reuse.es_name}}. For further information, see the list of [supported configuration options](https://strimzi.io/docs/operators/0.22.0/using.html#type-KafkaClusterSpec-reference){:target="_blank"}.
...
This release of {{site.data.reuse.es_name}} does not support the `secretPrefix` configuration property. The `secretPrefix` property adds a prefix to the name of all Secrets created from the `KafkaUser` resource. This can cause problems with the normal operation of {{site.data.reuse.es_name}} in this release.
