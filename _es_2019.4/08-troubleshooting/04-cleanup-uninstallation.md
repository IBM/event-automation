...
A service ID is created as part of installing {{site.data.reuse.es_name}}, which defines the identity for securing communication between internal components. To delete this service ID after uninstalling {{site.data.reuse.es_name}}, run the following command:
...
Use the following command to find the list of {{site.data.reuse.es_name}} objects associated with the release’s namespace:
...
{{site.data.reuse.es_name}} 2019.2.1 and earlier created a number of objects in the `kube-system` namespace. You can safely remove these objects after uninstalling.
...
Be cautious of deleting persistent volume claims (PVCs) as the data on the disk that is associated with that persistent volume will also be deleted. This includes {{site.data.reuse.es_name}} message data.
