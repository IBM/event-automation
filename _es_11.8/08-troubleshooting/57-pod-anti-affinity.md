...
In {{site.data.reuse.es_name}} 11.8.x, the default pod anti-affinity rules are not optimized for KRaft-based deployments. If no rules are configured, the Kubernetes scheduler might place multiple Kafka broker pods on the same node.
