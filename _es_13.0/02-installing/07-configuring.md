...
**Note:** When applying custom Kafka configuration settings to your {{site.data.reuse.es_name}}, check the [Kafka documentation](https://kafka.apache.org/42/documentation){:target="_blank"} to ensure the new configuration settings are consistent and do not cause conflicts.
...
For more information, see the [managing node pool IDs](https://strimzi.io/docs/operators/1.0.0/deploying#proc-managing-node-pools-ids-str){:target="_blank"}.
...
For more information about the limitations, see the [Strimzi KRaft documentation](https://strimzi.io/docs/operators/1.0.0/deploying#kraft_limitations){:target="_blank"}.
...
- If no storage class is specified and no default storage class has been defined in the {{site.data.reuse.openshift_short}} settings, the deployment will use any [persistent volume claims](https://v1-35.docs.kubernetes.io/docs/concepts/storage/persistent-volumes/){:target="_blank"} that have at least the set size value.
...
For more information about logging, see the [Kafka logging reference](https://strimzi.io/docs/operators/1.0.0/configuring.html#property-kafka-logging-reference){:target="_blank"}.
...
The format of the `affinity` property matches the [Kubernetes specification](https://v1-35.docs.kubernetes.io/docs/concepts/scheduling-eviction/assign-pod-node/#affinity-and-anti-affinity){:target="_blank"}. For example, if a node is labeled with `mykey=myvalue`, the `affinity` would contain the following settings:
...
**Note:** When geo-replication is set up through the {{site.data.reuse.es_name}} UI or CLI, a `KafkaMirrorMaker2` custom resource is created on the destination cluster. You can enable metrics by editing the `KafkaMirrorMaker2` custom resource.
...
For more information about configuration options, see [configuring the Kafka Exporter](https://strimzi.io/docs/operators/1.0.0/deploying.html#proc-metrics-kafka-deploy-options-str){:target="_blank"}.
...
- [Kafka JMX metrics configuration](https://strimzi.io/docs/operators/1.0.0/deploying.html#assembly-metrics-str){:target="_blank"}
- [Prometheus JMX metrics configuration](https://strimzi.io/docs/operators/1.0.0/configuring.html#con-common-configuration-prometheus-reference){:target="_blank"}
...
**Note:** Ensure you add the unit when configuring a `brokerCapacity` key.

For example, to configure Cruise Control to optimize around having an inbound network byte rate of `1000` kilobytes per second, configure the `spec.strimziOverrides.cruiseControl.brokerCapacity` property as follows:
