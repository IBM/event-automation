...
The following are examples of audit events that are logged by the {{site.data.reuse.eem_manager}} that show the event model in use:
...
2. `SIMPLE`: A succinct version of the `CADF` format that contains only essential elements. The previous [examples](#examples) were produced by using this format.
...
### {{site.data.reuse.egw}} audit logging environment variables
{: #egw-environment-variables}

Refer to the [gateway properties reference](../../reference/gateway-properties#audit-log) for full details of the gateway audit logging properties.

#### Operator-managed gateway
{: #opman-gway}

Audit logging properties are configured as environment variables in the {{site.data.reuse.egw}} custom resource. Refer to the [Event Gateway properties reference](../../reference/gateway-properties#audit-log).

#### Kubernetes Deployment gateway
{: #k8s-gateway}

Specify the audit logging format and writer in the {{site.data.reuse.egw}} ConfigMap properties: `audit.log.format` and `audit.log.writer`. 

Specify all other audit logging properties as environment variables in the {{site.data.reuse.egw}} Kubernetes Deployment.
