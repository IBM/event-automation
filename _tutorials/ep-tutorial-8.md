...
description: "App Connect is a good example of how to trigger automations and notifications from event processing results."
...
In this scenario, we will generate notifications in response to events recognized in a {{site.data.reuse.ep_name}} flow.
...
To show how this can be used, this tutorial shows how to use that topic in App Connect.
...
1. Check that the flow you created in the [Distributing results of analysis and processing](../guided/tutorial-5) tutorial is still running.
...
1. Add a message text, including fields from the parsed Kafka message
...
The flow is now ready to run. The final step is to start it running.

1. Click the **Start flow** button
...
1. Verify that new orders made in the EMEA region result in a notification in the selected Slack channel
...
App Connect is a good example of how to trigger automations and notifications from event processing results.
