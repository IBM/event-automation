...
You can upgrade {{site.data.reuse.es_name}} to the [latest 11.6.x version]({{ 'support/matrix/#event-streams' | relative_url }}) directly from any earlier 11.6.x or any 11.5.x version by using the latest 3.6.x operator. The upgrade procedure depends on whether you are upgrading to a major, minor, or patch level version, and what your catalog source is.

If you are upgrading from {{site.data.reuse.es_name}} version 11.4.x or earlier, you must first [upgrade your installation to 11.5.x]({{ 'es/es_11.5' | relative_url }}/installing/upgrading/) and then follow these instructions to upgrade from 11.5.x to 11.6.x.
...
- Support for IAM authentication in {{site.data.reuse.es_name}} version 11.6.0 and later has been removed. If you are using IAM for authentication, ensure you switch to SCRAM or Keycloak before upgrading to avoid authentication issues. To configure your instance, edit the `EventStreams` custom resource and [specify the authentication method](../../installing/configuring/#configuring-ui-and-cli-security) in the `spec.adminUI.authentication` section. For example, to use Keycloak, set the authentication `type` to `integrationKeycloak`.
...
- `<previous-version>` is the version of the Helm chart being upgraded from. For example, if your Helm chart version is 3.5.0, set the field as: `--set previousVersion=3.5.0`. You can retrieve the version of your existing Helm chart by running the following command:
...
After upgrading to {{site.data.reuse.es_name}} 11.6.0, perform the following tasks if the {{site.data.reuse.es_name}} instance was configured to use the Topic Operator before upgrading.
