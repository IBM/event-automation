...
A valid flow can terminate with multiple destination nodes ([event destination](../eventnodes/#event-destination) or [SQL destination](../custom/)), which can be connected from any node that is not itself a destination node.
