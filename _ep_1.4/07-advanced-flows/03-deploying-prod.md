...
- You updated the Flink SQL connector properties and values that are defined in the file `statements.sql` to match your target environment.
  ...
  | **HTTP** | [API](../../nodes/enrichmentnode/#enrichment-from-an-api) | [About HTTP connector](https://github.com/getindata/flink-http-connector/blob/0.25.0/README.md){:target="_blank"} | [HTTP connector properties](https://github.com/getindata/flink-http-connector/blob/0.25.0/README.md#table-api-connector-options){:target="_blank"}. <br> <br> **Note:** The HTTP connector versions: <br> <br> - ![Event Processing 1.4.7 icon]({{ 'images' | relative_url }}/1.4.7.svg "In Event Processing 1.4.7 and later.") 0.25.0 - {{site.data.reuse.ep_name}} versions 1.4.7 and later <br> <br> - ![Event Processing 1.4.7 icon]({{ 'images' | relative_url }}/1.4.5.svg "In Event Processing 1.4.5.") 0.22.0 - version 1.4.5 <br> <br> - 0.19.0 - versions 1.4.0 to 1.4.4, and 1.4.6 <br> <br> ![Event Processing 1.4.6 icon]({{ 'images' | relative_url }}/1.4.6.svg "In Event Processing 1.4.6.") (In {{site.data.reuse.ep_name}} 1.4.6, the HTTP connector version is downgraded due to an issue in 1.4.5) <br> <br>  |
...
1. Modify the [Dockerfile](https://github.com/apache/flink-kubernetes-operator/blob/main/examples/flink-sql-runner-example/Dockerfile){:target="_blank"} to use the IBM Flink image:
   ...
   a. Execute the following command to extract the Flink image name including its SHA digest from the `ClusterServiceVersion` (CSV). For example, if you are running on Flink version 1.4.7:
