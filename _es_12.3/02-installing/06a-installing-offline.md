...
- A supported version of {{site.data.reuse.openshift_short}} [installed](https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/overview/welcome-index){:target="_blank"}. For supported versions, see the [support matrix]({{ 'support/matrix/#event-streams' | relative_url }}).
...
2. Run the following command to download, validate, and extract the {{site.data.reuse.es_name}} CASE.
   ...
   - Success
   Download of CASE: ibm-eventstreams, version: 12.3.1 is complete
   ```
...
3. Verify that the CASE and images (`.csv`) files have been generated for {{site.data.reuse.es_name}}.
   ...
   │   │           ├── charts
   │   │           ├── ibm-eventstreams-12.3.1-airgap-metadata.yaml
   │   │           ├── ibm-eventstreams-12.3.1-charts.csv
   │   │           ├── ibm-eventstreams-12.3.1-images.csv
   │   │           ├── ibm-eventstreams-12.3.1.tgz
   │   │           └── resourceIndexes
...
**Note:** The `ImageContentSourcePolicy` API is deprecated in OpenShift 4.14 and later. Use `ImageDigestMirrorSet` instead for new deployments.

### Using `ImageDigestMirrorSet`
{: #using-imagedigestmirrorset}

**Important:** If you already have `ImageContentSourcePolicy` resources configured, see the [post-upgrade tasks](../upgrading/#migrate-imagecontentsourcepolicy-to-imagedigestmirrorset) for migration instructions before proceeding.

Complete the following steps to configure `ImageDigestMirrorSet`:
...
1. Ensure that an `ImageDigestMirrorSet` YAML file is [created](#mirror-the-images) for {{site.data.reuse.es_name}}. For example:
   ...
   spec:
     imageDigestMirrors:
     - mirrors:
       - <target-registry>/cp/ibm-eventstreams
       source: cp.icr.io/cp/ibm-eventstreams
     - mirrors:
...
1. Apply the `ImageDigestMirrorSet` custom resources:
   ...
   ```shell
   oc apply -f ~/.ibm-pak/data/mirror/ibm-eventstreams/<case-version>/image-digest-mirror-set.yaml
   ```
