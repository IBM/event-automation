...
Create a namespace into which the {{site.data.reuse.es_name}} instance will be installed by creating a [project](https://docs.redhat.com/en/documentation/openshift_container_platform/4.22/html/building_applications/projects#working-with-projects){:target="_blank"}.
...
To make the {{site.data.reuse.es_name}} operator available in the OpenShift software catalog, create the following YAML files and apply them as follows.
...
If you are installing the {{site.data.reuse.es_name}} operator as the first IBM operator in your cluster, create and apply the following YAML file to make the operators available in the OpenShift software catalog.
...
These steps add the catalog source for {{site.data.reuse.es_name}} to the software catalog, making the operator available to install.
