...
To allow communication between {{site.data.reuse.apic_short}} and {{site.data.reuse.eem_name}}, you must add the certificate presented on the `jwksUrl` endpoint to {{site.data.reuse.eem_name}} as a trusted certificate. Additionally, you must provide a JWKS endpoint, which will be used to authenticate messages received from {{site.data.reuse.apic_short}}.

1. Download the server certificate from an {{site.data.reuse.apic_short}} endpoint, either by opening the URL in a browser, or by running the following command and then copying the certificate details into a file:

   ```bash
   openssl s_client -connect <jwksUrl value>
   ```

   Where `<jwksUrl value>` is the {{site.data.reuse.apic_short}} `jwksUrl` endpoint that you retrieved earlier.
2. In the Kubernetes cluster running {{site.data.reuse.eem_name}}, create a secret that contains the downloaded certificate. Create a secret to store the {{site.data.reuse.apic_short}} certificate as follows.
...
3. Update the `EventEndpointManagement` instance with the {{site.data.reuse.apic_short}} configuration details:
