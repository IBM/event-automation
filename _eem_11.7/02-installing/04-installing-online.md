...
If you are installing the {{site.data.reuse.eem_name}} operator as the first IBM operator in your cluster, to make the operators available in the OpenShift software catalog, create the following YAML file and apply it as follows.
...
The preceding steps add the catalog source for {{site.data.reuse.eem_name}} to the software catalog, making the operator available to install.
...
3. Create a `Subscription` for the {{site.data.reuse.eem_name}} operator as follows:
   ...
   - `<current_channel>` is the operator channel for the release you want to install (see the [support matrix]({{ 'support/matrix/#event-endpoint-management' | relative_url }})). For example: "v11.7".
...
{: #checking-operator-status-by-using-the-web-console}
...
#### By using the CLI
{: #checking-operator-status-by-using-the-cli}
...
{: #installing-an-manager-instance-by-using-the-web-console}
...
{: #installing-an-event-endpoint-management-instance-by-using-the-yaml-view}
...
- Optional: If you are installing with a usage-based license, ensure that you [copy the secrets]({{ 'support/licensing#additional-steps-for-usage-based-license' | relative_url }}) created by the IBM License Service before you install the {{site.data.reuse.eem_manager}} instance, and then complete the following steps to provide details of your license service:
  ...
  ```yaml
  extensionServices:
    - endpoint: https://ibm-licensing-service-instance-ibm-common-services.apps.eem-cluster.cp.fyre.ibm.example.com
      name: licensing-service
...
{: #installing-an-event-endpoint-management-instance-by-using-the-form-view}
...
3. In **Manager > storage > type**, select `ephemeral` or `persistent-claim` based on your requirements. See [configuring persistence](../configuring/#enabling-persistent-storage) for information about storage types, and other optional specifications such as storage size, root storage path, and secrets.
4. In **Manager > Advanced Configuration > TLS**, use one of the following configurations:
...
{: #installing-an-manager-instance-by-using-the-cli}
